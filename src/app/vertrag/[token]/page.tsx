...
function isAlreadySigned(data: FetchResult): data is AlreadySignedData {
  return "alreadySigned" in data && data.alreadySigned === true;
}
...
export default function VertragPage() {
    ...
    fetch(`/api/vertrag/${token}`)
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(data.error || "Vertrag nicht gefunden");
        }
        return res.json();
      })
      .then((data: FetchResult) => {
        if (isAlreadySigned(data)) {
  ...
  function startDraw(cx: number, cy: number) {
    if (!canvasRef.current) return;
  ...
  function moveDraw(cx: number, cy: number) {
  ...
  async function handleSign() {
    if (!canSign) return;
    if (!hasDrawn.current) {
      setSignError("Bitte zeichnen Sie Ihre Unterschrift im Feld unten.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setSignError(data.error || "Fehler beim Unterschreiben");
        return;
      }
      setSigned(true);
    } catch {
      setSignError("Verbindungsfehler. Bitte versuchen Sie es erneut.");
    } finally {
      setSigning(false);
    }
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-10 text-center">
          ...
          <p className="text-slate-500">
            Dieser Vertrag wurde am <strong>{new Date(alreadySigned.signedAt).toLocaleDateString("de-DE")}</strong> von{" "}
            <strong>{alreadySigned.signedByName}</strong> unterschrieben.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-emerald-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-10 text-center">
          ...
          {contract?.offerToken && (
            <a
              href={`/api/angebot/${contract.offerToken}/pdf`}
              target="_blank"
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="px-8 py-6 border-b border-slate-100">
            ...
            <div className="bg-slate-50 rounded-xl p-5 text-sm text-slate-600 leading-relaxed space-y-3">
              ...
              <p><strong>§4 Stornierung:</strong> Bis 7 Tage vor Termin: 30%. 6-3 Tage: 50%. 2 Tage: 80%. Am Leistungstag oder bei Nichterscheinen: 100%.</p>
              ...
              <p><strong>§6 Schlussbestimmungen:</strong> Es gilt deutsches Recht. Änderungen bedürfen der Textform. Gerichtsstand ist Berlin.</p>
            </div>
