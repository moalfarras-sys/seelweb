"use client";

import Link from "next/link";
import { motion } from "framer-motion";
import { ArrowRight, Check, Truck, Shield, Clock, Package, Users, MapPin } from "lucide-react";
import ScrollReveal from "@/components/ui/ScrollReveal";

const benefits = [
  { icon: Truck, title: "Moderne Fahrzeuge", desc: "3,5-Tonnen-LKW mit Hebebühne für sichere Transporte" },
  { icon: Users, title: "Erfahrenes Team", desc: "Geschulte Mitarbeiter mit langjähriger Erfahrung" },
  { icon: Shield, title: "Versichert", desc: "Transportversicherung gemäß HGB §451e inklusive" },
];

const features = [
  "Möbelmontage und -demontage",
  "Verpackungsmaterial auf Wunsch",
  "Halteverbotszone einrichten",
  "Küchenmontage/-demontage",
  "Schwertransporte (Klavier, Tresor)",
  "Einlagerung auf Wunsch",
  "Besenreine Übergabe der alten Wohnung",
  "Flexible Terminwahl",
];

const faqs = [
  { q: "Wie wird der Umzugspreis berechnet", a: "Der Preis setzt sich aus Arbeitszeit, Anzahl der Mitarbeiter, Entfernung, Volumen und Stockwerkzuschlägen (ohne Aufzug) zusammen. Alle Positionen werden transparent im Angebot aufgeführt." },
  { q: "Wie weit im Voraus sollte ich buchen", a: "Wir empfehlen eine Buchung mindestens 2 Wochen im Voraus. Für Expressumzüge bieten wir auch kurzfristige Termine an." },
  { q: "Muss ich selbst Kartons besorgen", a: "Nein, wir können Ihnen Umzugskartons und Verpackungsmaterial gegen Aufpreis bereitstellen. Sprechen Sie uns bei der Buchung darauf an." },
  { q: "In welchem Gebiet sind Sie tätig", a: "Wir führen Umzüge in Deutschland durch. Fernumzüge deutschlandweit sind auf Anfrage ebenfalls möglich." },
];

export default function UmzugPage() {
  return (
    <>
      <section className="gradient-navy py-20 md:py-28 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10 pointer-events-none">
          <div className="absolute top-10 right-20 w-72 h-72 bg-teal-500 rounded-full blur-[128px]" />
        </div>
        <div className="max-w-5xl mx-auto px-4 md:px-8 text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="w-20 h-20 bg-teal-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6">
              <Truck className="text-teal-400" size={40} />
            </div>
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Privatumzug deutschlandweit</h1>
            <p className="text-silver-300 max-w-2xl mx-auto text-lg">Stressfrei umziehen mit Profis. Wir kümmern uns um alles – vom Einpacken bis zum Aufbau in der neuen Wohnung.</p>
            <Link href="/buchen?service=MOVING" className="inline-flex items-center gap-2 mt-8 px-8 py-4 bg-teal-500 text-white font-semibold rounded-xl hover:bg-teal-600 transition-all shadow-lg shadow-teal-500/25">
              Jetzt Umzug buchen <ArrowRight size={18} />
            </Link>
          </motion.div>
        </div>
      </section>

      <section className="section-padding bg-white dark:bg-navy-950">
        <div className="max-w-5xl mx-auto">
          <ScrollReveal>
            <h2 className="text-2xl md:text-3xl font-bold text-navy-800 dark:text-white mb-10 text-center">Ihre Vorteile</h2>
            <div className="grid md:grid-cols-3 gap-6">
              {benefits.map((b) => {
                const Icon = b.icon;
                return (
                  <div key={b.title} className="bg-gray-50 dark:bg-navy-800/60 rounded-2xl p-6 border border-gray-100 dark:border-navy-700/50">
                    <div className="w-12 h-12 bg-teal-500/10 rounded-xl flex items-center justify-center mb-4">
                      <Icon className="text-teal-500" size={24} />
                    </div>
                    <h3 className="font-bold text-navy-800 dark:text-white mb-2">{b.title}</h3>
                    <p className="text-sm text-silver-600 dark:text-silver-300">{b.desc}</p>
                  </div>
                );
              })}
            </div>
          </ScrollReveal>
        </div>
      </section>

      <section className="section-padding bg-gray-50 dark:bg-navy-900">
        <div className="max-w-5xl mx-auto">
          <ScrollReveal>
            <h2 className="text-2xl md:text-3xl font-bold text-navy-800 dark:text-white mb-8 text-center">Was ist enthalten</h2>
            <div className="grid md:grid-cols-2 gap-4">
              {features.map((f) => (
                <div key={f} className="flex items-start gap-3 bg-white dark:bg-navy-800/60 rounded-xl p-4 border border-gray-100 dark:border-navy-700/50">
                  <Check size={18} className="text-teal-500 mt-0.5 shrink-0" />
                  <span className="text-sm text-navy-700 dark:text-silver-200">{f}</span>
                </div>
              ))}
            </div>
          </ScrollReveal>
        </div>
      </section>

      <section className="section-padding bg-white dark:bg-navy-950">
        <div className="max-w-3xl mx-auto text-center">
          <ScrollReveal>
            <h2 className="text-2xl font-bold text-navy-800 dark:text-white mb-4">Transparente Preisgestaltung</h2>
            <p className="text-silver-600 dark:text-silver-300 mb-4">Unser Umzugspreis setzt sich transparent zusammen aus Arbeitszeit, Mitarbeiteranzahl, Entfernung (km), Ladevolumen (m³) und ggf. Stockwerkzuschlägen.</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              {[
                { icon: Clock, label: "Stundenbasiert" },
                { icon: MapPin, label: "km-genau" },
                { icon: Package, label: "Nach Volumen" },
                { icon: Shield, label: "Versichert" },
              ].map((item) => {
                const Icon = item.icon;
                return (
                  <div key={item.label} className="bg-teal-50 dark:bg-teal-500/10 rounded-xl p-4 text-center">
                    <Icon className="text-teal-600 mx-auto mb-2" size={24} />
                    <span className="text-sm font-medium text-teal-800 dark:text-teal-300">{item.label}</span>
                  </div>
                );
              })}
            </div>
          </ScrollReveal>
        </div>
      </section>

      <section className="section-padding bg-gray-50 dark:bg-navy-900">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl font-bold text-navy-800 dark:text-white mb-8 text-center">Häufige Fragen</h2>
          <div className="space-y-3">
            {faqs.map((faq) => (
              <details key={faq.q} className="group bg-white dark:bg-navy-800/60 rounded-xl border border-gray-100 dark:border-navy-700/50">
                <summary className="px-5 py-4 cursor-pointer font-medium text-navy-800 dark:text-white text-sm flex items-center justify-between">
                  {faq.q}
                  <span className="text-teal-500 transition-transform group-open:rotate-45 text-lg">+</span>
                </summary>
                <div className="px-5 pb-4 text-sm text-silver-600 dark:text-silver-300">{faq.a}</div>
              </details>
            ))}
          </div>
        </div>
      </section>

      <section className="gradient-navy section-padding">
        <div className="max-w-4xl mx-auto text-center">
          <ScrollReveal>
            <h2 className="text-3xl font-bold text-white mb-4">Bereit für Ihren Umzug</h2>
            <p className="text-silver-300 mb-8 max-w-xl mx-auto">Erhalten Sie ein unverbindliches Angebot in wenigen Minuten. Transparent, fair und professionell.</p>
            <Link href="/buchen?service=MOVING" className="inline-flex items-center gap-2 px-10 py-4 bg-teal-500 text-white font-semibold rounded-xl hover:bg-teal-600 transition-all shadow-lg shadow-teal-500/25">
              Jetzt Angebot anfordern <ArrowRight size={20} />
            </Link>
          </ScrollReveal>
        </div>
      </section>
    </>
  );
}

